...
export function ProgramModal({ program, open, onClose }: ProgramModalProps) {
  ...
  const budgetUtilizationValue = program.budgetAllocated ? Math.round((program.budgetUsed / program.budgetAllocated) * 100) : 0;
  const successRateValue = 92; // Mock success rate
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {t("Last updated")}: {formatDate(program.updatedAt || new Date())}
                </p>
              </div>
              <Badge 
                variant="secondary"
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-muted-foreground">
                    {program.description || t("No description available")}
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h4 className="font-medium mb-2">{t("Timeline")}</h4>
                    <p className="text-sm text-muted-foreground">
                      {program.startDate ? formatDate(program.startDate) : "TBD"} - {program.endDate ? formatDate(program.endDate) : "TBD"}
                    </p>
                  </div>
                  <div>
                    <h4 className="font-medium mb-2">{t("Budget")}</h4>
                    <p className="text-sm text-muted-foreground">
